...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Quadrans peer", "peer", id)
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Quadrans protocol")
	...
	log.Info("Quadrans protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Quadrans peer connected", "name", p.Name())
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Quadrans handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Quadrans peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Quadrans message handling failed", "err", err)
			return err
		}
	}
}
